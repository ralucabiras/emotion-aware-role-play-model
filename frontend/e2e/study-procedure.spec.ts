...
test('enrolled participant follows all three guided tasks at intermediate difficulty', async ({page})=>{
  ...
  for(const [index,id,title] of [[0,'workload','Workload conversation'],[1,'boundary','Boundary conversation'],[2,'relationship','Relationship conversation']] as const){
    ...
    expect((await start).postDataJSON()).toMatchObject({scenario_id:id,difficulty:'intermediate',attempt_purpose:'required',required_task_id:id})
...
test('retry keeps an incomplete required task incomplete', async ({page}) => {
  await installApiMock(page, {enrolled:true})
  await page.goto('/practice?mode=roleplay&study=workload')
  await page.getByRole('button', {name:'Skip pre-ratings and begin'}).click()
  await page.getByRole('button', {name:'Finish & review'}).click()
  ...
  await expect(page.getByText('This retry is additional practice and will not replace your required study attempt.')).toBeVisible()
  const start = page.waitForRequest(request => request.url().endsWith('/session-1/roleplay'))
  await page.getByRole('button', {name:'Skip pre-ratings and begin'}).click()
  expect((await start).postDataJSON()).toMatchObject({attempt_purpose:'retry',required_task_id:'workload'})
  await page.getByRole('button', {name:'Finish & review'}).click()
  await page.getByLabel('Confidence now').selectOption('5')
  await page.getByLabel('Scenario realism').selectOption('6')
  await page.getByLabel('Feedback usefulness').selectOption('7')
...
test('switching from checklist to additional practice sends no required association', async ({page}) => {
  await installApiMock(page, {enrolled:true})
  await page.goto('/practice?mode=roleplay&study=workload')
  await page.getByRole('button', {name:'Switch to additional practice'}).click()
  const start = page.waitForRequest(request => request.url().endsWith('/session-1/roleplay'))
  await page.getByRole('button', {name:'Skip pre-ratings and begin'}).click()
  expect((await start).postDataJSON()).toMatchObject({attempt_purpose:'additional',required_task_id:null})
})
