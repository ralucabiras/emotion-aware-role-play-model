...
export default function App() {
  ...
  const practice = (profile:UserProfile) => { const query=new URLSearchParams(location.search);return <Dashboard user={profile} initialSessionId={query.get('session')??undefined} initialRoleplay={query.get('mode')==='roleplay'} initialStudyTask={query.get('study')??undefined} onLogout={() => { setUser(undefined); navigate('/') }} onDashboard={() => navigate('/app')} onSettings={() => navigate('/settings')}/> }
