...
export function ScenarioSetup({studyTask,onFreePractice,scenarios, selected, difficulty, busy, onScenario, onDifficulty, onStart, onScenarios}: {studyTask?:string;onFreePractice?:()=>void;scenarios: Scenario[]; selected: string; difficulty: string; busy: boolean; onScenario: (id: string) => void; onDifficulty: (level: string) => void; onStart: (ratings: {confidence:number;anxiety:number}|null) => void; onScenarios:(items:Scenario[])=>void}) {
  ...
  async function remove(id:string){
    if(removing||busy||saving||!confirm('Delete this custom scenario? Existing rehearsal history will remain available.'))return
    setRemoving(true);setRemoveError('')
    try{await api.deleteScenario(id);const remaining=scenarios.filter(item=>item.id!==id);onScenarios(remaining);if(selected===id)onScenario(remaining[0]?.id??'workload')}
    catch(caught){setRemoveError(caught instanceof Error?caught.message:'Scenario could not be deleted. Please try again.')}
  ...
  return <section className="scenario-setup">
    ...
    <div className="scenario-cards">{scenarios.filter(item=>!studyTask||item.id===studyTask).map(item => <div className="scenario-card-wrap" key={item.id}><button disabled={removing} className={selected === item.id ? 'selected' : ''} onClick={() => onScenario(item.id)} aria-pressed={selected === item.id}><span className="scenario-icon">{item.title.charAt(0)}</span><strong>{item.title}</strong><small>Practise with a {item.character}</small>{item.id.startsWith('custom_')&&<em>Your scenario</em>}</button>{item.id.startsWith('custom_')&&<button disabled={busy||saving||removing} aria-busy={removing} className="remove-scenario" onClick={()=>void remove(item.id)} aria-label={`Delete ${item.title}`}>×</button>}</div>)}</div>
