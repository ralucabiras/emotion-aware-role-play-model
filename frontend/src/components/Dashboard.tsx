...
export function Dashboard({user, initialSessionId, initialRoleplay=false, onLogout, onDashboard, onSettings}: {user: UserProfile; initialSessionId?:string; initialRoleplay?:boolean; onLogout: () => void; onDashboard:()=>void; onSettings: () => void}) {
  ...
  const [selected, setSelected] = useState(() => recommendedScenario(user.practice_goals))
  ...
  async function submit(event: FormEvent) {
    ...
    try { const response = await api.sendMessage(sessionId, content); setTurns(current => [...current, response.turn]); setEmotion(response.decision.emotion_state); setRoleplay(response.roleplay); setFeedback(response.feedback); setPostToken(response.post_questionnaire_token??null); if (sessionTitle === 'New reflection') setSessionTitle(content.length > 57 ? `${content.slice(0,57).trim()}…` : content); if (response.feedback) setMode('feedback') }
  ...
  async function start(ratings: {confidence:number;anxiety:number}|null) {
    ...
    try { const response = await api.startRoleplay(sessionId, selected, difficulty, ratings); setSessionId(response.session_id); history.replaceState({}, '', `/practice?session=${response.session_id}`); setEmotion(response.emotion_state); setSessionTitle(response.scenario.title); setTurns([response.opening_turn]); setRoleplay(response.state); setFeedback(null); setMultimodal(null); storeVoiceSample(null); setMessage(''); setTranscriptionStatus('idle'); setMode('roleplay'); setSessions(await api.listSessions()) } catch(caught) { setError(caught instanceof Error?caught.message:'The rehearsal could not be started.') } finally { setBusy(false) }
  }
  async function action(name: string, nextMode: WorkspaceMode = 'roleplay') { if (!sessionId) return; setBusy(true); setError(''); try { const session = await api.roleplayAction(sessionId, name); load(session); setMode(session.feedback ? 'feedback' : nextMode) } catch(caught) { setError(caught instanceof Error?caught.message:'The role-play could not be updated.') } finally { setBusy(false) } }
  ...
  return <main className="shell" aria-busy={busy}>
    ...
    <ModeTabs mode={mode} roleplay={roleplay} onChange={next => { if(next!=='feedback')setPostToken(null); if (roleplayActive && next === 'reflect' && !confirm('Pause the active role-play and return to reflection?')) return; if (roleplayActive && next === 'reflect' && roleplay?.status === 'active') { void action('pause', 'reflect'); return } setMode(next) }}/>
    {error && <p className="error" role="alert">{error}</p>}
    {mode === 'feedback' && feedback ? <FeedbackScreen scenario={activeScenario} feedback={feedback} state={roleplay} postToken={postToken} onRetry={retry} onRewind={()=>void rewind()} onConversation={() => {setPostToken(null);setMode('reflect')}}/> : mode === 'roleplay' && !roleplayActive ? <ScenarioSetup scenarios={scenarios} selected={selected} difficulty={difficulty} busy={busy} onScenario={setSelected} onDifficulty={setDifficulty} onStart={start} onScenarios={setScenarios}/> : <div className="workspace"><section className={`chat-card ${roleplayActive ? 'roleplay-chat' : ''}`}>{roleplayActive && roleplay && <ActiveRolePlayHeader scenario={activeScenario} state={roleplay} busy={busy} onAction={action} onRewind={()=>void rewind()}/>}<div className="notice"><strong>{roleplayActive ? 'Role-play in progress' : 'Research prototype'}</strong><span>{roleplayActive ? `The assistant is responding as your ${activeScenario?.character ?? 'practice partner'}.` : 'Not a therapist or medical service. In an emergency, contact local emergency services.'}</span></div><div className="messages" aria-live="polite">{turns.length === 0 && <div className="empty"><span>✦</span><h2>What conversation is on your mind?</h2><p>Your affect estimate is uncertain and is never a diagnosis.</p></div>}{turns.map(turn => <div key={turn.id} className={`message ${turn.role}`}><span>{turn.content}</span></div>)}{busy && <div className="message assistant"><span>Thinking…</span></div>}<div ref={endRef}/></div>{voiceNotice && <p className="voice-notice" role="status">{voiceNotice}</p>}{composer}</section><aside><EmotionPanel state={emotion}/>{multimodal && <MultimodalPanel result={multimodal}/>}<button className="new-session" onClick={fresh}>Delete & start fresh</button></aside></div>}
  </main>
}
