...
export function VoiceCapture({ enabled, disabled, sample, onChange }: { enabled: boolean; disabled: boolean; sample: VoiceSample | null; onChange: (sample: VoiceSample | null) => void }) {
  ...
  return <div className={`voice-capture ${displayStatus}`} aria-busy={displayStatus==='recording'}>
    ...
    {displayStatus === 'ready' && <button type="button" className="remove-audio" aria-label="Remove attached voice sample" onClick={()=>{onChange(null);setStatus('idle');setSeconds(0)}}>Remove</button>}
