...
export interface SessionResponse { session_id: string; title: string; turns: ConversationTurn[]; emotion_state: EmotionState; roleplay: RolePlayState | null; feedback: Feedback | null }
export interface SessionSummary { session_id: string; title: string; created_at: string; updated_at: string; turn_count: number; roleplay?: RolePlayState | null; feedback?: Feedback | null }
export interface UserProfile { id: string; email: string; first_name: string; last_name: string; preferred_name: string; country: string; timezone: string; email_verified: boolean; practice_goals: string[]; onboarding_completed: boolean; onboarding_version?: string | null }
