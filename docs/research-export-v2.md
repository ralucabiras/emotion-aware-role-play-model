...
Use the same session's pre/post confidence for paired changes. Restrict primary task analysis to the protocol's required scenarios and intermediate difficulty, and check valid completion reasons and complete post-ratings. `safety_interruption` is not successful completion. Retain repeated attempts separately and apply the protocol's exploratory/retry rules in analysis.
