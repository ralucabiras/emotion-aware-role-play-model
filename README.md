...
The researcher dashboard and CSV export deliberately exclude names, email addresses, passwords, conversation text, audio, custom scenario wording, and saved takeaways. Only the first explicitly required attempt per task counts toward protocol completion; later practice cannot replace an incomplete required attempt. CSV v3 rows distinguish required attempts, additional practice, retries, and unclassified legacy records, and contain pseudonymous participant/session identifiers, enrollment and measurement timestamps, turn counts, scenario identifiers, difficulty, completion reason, questionnaire ratings, numeric skill scores and evidence-turn indices, feedback generation source, and response-source/fallback counts. Participants with no retained study sessions receive a participant-only row so they remain in the enrollment denominator. See the [research export data dictionary](docs/research-export-v3.md) for row types, missing values, and analysis guidance. Keep `PILOT_ACCESS_CODE` and the researcher allowlist private; neither replaces normal account authentication.
...
Rehearsal questionnaires require an explicit choice: answer every item or skip. Pre-ratings are recorded with the start request; post-ratings can be submitted only from the newly completed rehearsal's feedback screen. Reopening feedback shows the original read-only ratings or skip decision and does not reopen the submission window. Leaving the screen closes unanswered post-ratings. Skips and window closure are recorded as text-free events, never fabricated numeric scores. Once post-ratings or a skip are recorded, further practice requires a new attempt rather than rewinding the measured attempt.

Enrolled participants see a three-task checklist on the home dashboard and in the workspace: workload, boundary, then relationship. The next-task action fixes intermediate difficulty and the required scenario; additional practice remains available separately. Progress comes from durable study records and the same completion criteria as the researcher dashboard. Active tasks can be resumed. A skipped/closed questionnaire or interrupted attempt stays labelled incomplete while the guide allows progression to the remaining tasks; it is never counted as protocol completion. The guide waits for a fresh post-rating decision before offering the next task and disables study actions outside the collection window or after dataset freeze.
